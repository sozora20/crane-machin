...
type Phase =
  | 'idle' | 'moving' | 'descending' | 'closing'
  | 'ascending' | 'awaiting_result' | 'prize_fly' | 'done'
...
export default function ClawMachine({ grabTrigger, onResolveGrab, isGrabbing, result, onAnimationComplete }: Props) {
  ...
  const initSpheres = useCallback((w: number, h: number) => {
    ...
    spheres.current = Array.from({ length: SPHERE_COUNT }, (_, i) => ({
      ...
      y: floorY - r * 0.7 - Math.random() * r * 5.8,
      radius: r * (0.75 + Math.random() * 0.48),
  ...
  const findCaughtSphere = useCallback((cx: number, clawY: number, armW: number): number => {
    let best = -1, bestDist = Infinity
    for (let i = 0; i < spheres.current.length; i++) {
      const s = spheres.current[i]
      const halfW = armW - s.radius - 1.5
      if (halfW <= 0) continue
      const dx = Math.abs(s.x - cx)
      if (dx > halfW) continue
      const dy = Math.abs(s.y - (clawY + 20))
      if (dy > s.radius * 1.55) continue
      const dist = dx * 1.2 + dy
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const top = h * 0.03, bottom = h * 0.88
      const floorY = h * 0.77
      ctx.save()
      ...
      strip.addColorStop(0, 'rgba(230,184,255,0.6)')
      strip.addColorStop(0.33, 'rgba(122,162,247,0.6)')
      strip.addColorStop(0.66, 'rgba(255,107,157,0.6)')
      strip.addColorStop(1, 'rgba(255,215,0,0.6)')
      ctx.fillStyle = strip
      ctx.beginPath(); (ctx as any).roundRect(left, top, right - left, 5, [18, 18, 0, 0]); ctx.fill()

      // Dot grid
      ctx.fillStyle = 'rgba(187,154,247,0.055)'
      for (let gx = left + 24; gx < right; gx += 30)
        for (let gy = top + 24; gy < floorY; gy += 30) {
          ctx.beginPath(); ctx.arc(gx, gy, 1.2, 0, Math.PI * 2); ctx.fill()
        }

      // Floor / chute
      ctx.fillStyle = 'rgba(8,8,18,0.96)'; ctx.fillRect(left, floorY, right - left, bottom - floorY)
      ctx.strokeStyle = 'rgba(187,154,247,0.20)'; ctx.lineWidth = 1
      ...
      ctx.save()
      ctx.shadowColor = 'rgba(255,215,0,0.35)'; ctx.shadowBlur = 12
      ctx.strokeStyle = 'rgba(255,215,0,0.32)'; ctx.lineWidth = 1.5
      ctx.beginPath()
      ctx.moveTo(chuteX, bottom - 2)
      ctx.lineTo(chuteX + 12, bottom + 18)
      ctx.lineTo(chuteX + chuteW - 12, bottom + 18)
      ctx.lineTo(chuteX + chuteW, bottom - 2)
      ctx.stroke()
      ctx.restore()

      // Claw X smooth movement
      ...
      if (ph === 'descending') {
        ...
        for (const s of spheres.current) {
          ...
          if (Math.abs(sdx) < armW && sdy > -s.radius && sdy < 32 + s.radius) {
            s.vx += sdx >= 0 ? 0.55 : -0.55; s.vy -= 0.15
          }
        }
      ...
      if (ph === 'closing') {
        ...
        const targetScale = caughtRadius.current > 0
          ? Math.max(0.08, (caughtRadius.current * 0.82) / BASE_RADIUS)
          : 0.08
        clawScale.current = Math.max(clawScale.current - 0.055, targetScale)

        if (closingFrame.current === 8) {
          const cy = trackY + 30 + clawDescent.current
          const found = findCaughtSphere(cx, cy, armW)
          if (found >= 0) {
            ...
            for (const s of spheres.current) {
              const sdx = s.x - cx
              if (Math.abs(sdx) < 40) { s.vx += (sdx > 0 ? 4 : -4) + (Math.random() - 0.5) * 2; s.vy -= 2.5 + Math.random() * 2 }
            }
          }
          if (!resolvedRef.current) {
            onResolveGrab(Math.round(clawX.current * 100), found >= 0)
      ...
      if (ph === 'prize_fly') {
        ...
        if (ci >= 0 && spheres.current[ci]) {
          ...
          if (closingFrame.current >= 28) {
            for (let i = 0; i < 22; i++) particles.current.push({ x: s.x, y: s.y, vx: (Math.random() - 0.5) * 7, vy: Math.random() * -6 - 1, life: 1, colorIdx: s.colorIdx })
            if (particles.current.length > 180) particles.current = particles.current.slice(-180)
            spheres.current.splice(ci, 1); caughtIdx.current = -1; phase.current = 'done'; closingFrame.current = 0
          }
        } else { phase.current = 'done'; closingFrame.current = 0 }
      }
      ...
      for (let i = 0; i < spheres.current.length; i++) {
        ...
        for (let j = i + 1; j < spheres.current.length; j++) {
          ...
          const minDist = s.radius + b.radius
          if (dist < minDist && dist > 0.01) {
            const nx = ddx / dist, ny = ddy / dist
            const overlap = (minDist - dist) * 0.5
            s.x -= nx * overlap; s.y -= ny * overlap; b.x += nx * overlap; b.y += ny * overlap
            const relV = (s.vx - b.vx) * nx + (s.vy - b.vy) * ny
            if (relV > 0) { s.vx -= relV * nx * 0.52; s.vy -= relV * ny * 0.52; b.vx += relV * nx * 0.52; b.vy += relV * ny * 0.52 }
          }
        }
      }
      ...
      ctx.strokeStyle = 'rgba(187,154,247,0.28)'; ctx.lineWidth = 1.5
